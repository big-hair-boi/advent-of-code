...
using ExecutedOperation = std::tuple<Operation*, bool /* op changed */, int /* stack index */>;
...
RecursivelyFixProgram(const std::vector<Operation>& op_list,
  ...
  if (visited_ops.find(index) != visited_ops.end()) {
    // We've looped, so end the recursive search and update callers with the status
  ...
  const auto& recursive_results =
      RecursivelyFixProgram(op_list, visited_ops, new_index, op_has_been_changed);
...
void ComputePart2Recursive(const std::vector<Operation>& op_list) {
  ...
  assert(value.second == false /* validate there was eventually no recursive loop */);
  std::cout << "Part 2 - Final Accumulator value after fix: " << std::to_string(value.first);
}
