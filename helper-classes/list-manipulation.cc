...
std::pair<long long int, long long int> Get2Sum(long long int target_value,
  ...
  for (const auto &value : value_list) {
    if (value == used_value)
      continue;
    complement_map.insert(std::pair<long long int, long long int>(target_value - value, value));
  }
...
Get3Sum(long long int target_sum, const std::vector<long long int> &int_list) {
  ...
  for (const auto& value : int_list) {
    complement_map.insert(std::pair<long long int, long long int>(target_sum - value, value));
  }
